# Extended Bilstone html template parser
#
#		Old Modul: HTMLTMPL
#		Old Version 1.33	production
#		Old Author	Ian Steel
#
#		Old History from HTMLTMPL:
...
#
#		As new modul HTMLTMPL::Extended: (by Stefan Gipper, stefanos@cpan.org)
#			Sept. 2011 - 1.0 - First Release / Added include parse.
#

package HTMLTMPL::Extended;
...
sub version(){
	return $HTMLTMPL::Extended::VERSION;
}

sub srcData(){
	my $self = shift;
	return join("",@{$self->{'_C__HTML__'}});
}

sub srcInclude(){
	if($#_ != 1){
		bailout("Error! template data function requires a single parameter\n");
	}
	my $self = shift;
	my $src = shift;
	$src =~ s/__includes_(.+)__/addInclude($1)/eg;
...
sub srcString(){
	if($#_ != 1){
		bailout("Error! template string function requires a single parameter\n");
	}

	my $self = shift;
	my $src = $self->srcInclude(shift);#new with includes
	#my $src = shift;#old
...
sub parseSegment(){
	...
	while($tmplString =~ /(.*?)((__[ix]_.+?__\n)|(__.+?__))/sg){
		...
		if(defined $token){
			...
			push @$allToks, ($repeating ? $segHash->{'_C__ID__'} . ":" . $tokId : $tokId);
		}
	}
...
sub AUTOLOAD {
	...
	if(exists $self->{$tok}){
		...
		#bailout("Invalid token '$token'.<br>Maybe it needs qualifying within a " ."block?");
	}
}

sub setAll {
	my ($self, $tok, $value) = @_;
	$self->{'__'.$tok.'__'} =  encode_entities($value);
}
...
sub mergeData(){
	...
	if($repeating){
		# determine number of times we need to repeat putting out this segment.
		my $key;
...
sub listAllTokensGlobal(){
	my $self = shift;

	my @returnarray;
	my $html = $self->srcData();
	while($html =~ /__([^_]+)__/ig){
		push(@returnarray,$1);
	}
	return(\@returnarray);
}
...
# If called in a scalar context, returns a comma seperated list of tokens found within the template.
# If called in array context, returns array of tokens found within the template.
sub getAllTokens(){
	my $self = shift;
...
HTMLTMPL::Extended - Merges runtime data with static HTML template file.
...
The HTMLTMPL::Extended module can address the following template scenarios :
...
Merges the data already passed to the HTMLTMPL::Extended instance with the template file
specified in src().
...
Returns a string of html produced by merging the data passed to the HTMLTMPL::Extended
...
Returns an HTMLTMPL::Extended object which represents the repeating / invisible block of
...
The code :
 ...
 # Create a template object and load the template source.
 my($templ) = new HTMLTMPL::Extended;
 ...
 my $subTmpl = new HTMLTMPL::Extended;
